...
#define INIT_SETTINGS_COLOR(key, val) do {INIT_SETTINGS(key, val); defaultColorsList[key] = val;} while (false)
#define INIT_SETTINGS_FONT(key, val) do {INIT_SETTINGS(key, val); defaultFontsList[key] = val;} while (false)
...
inline QMap<QString, QColor> defaultColorsList = {};
inline QMap<QString, QFont> defaultFontsList = {};
...
inline int levenshteinDistance(QString first, QString second)
{
    if (first.length() > second.length())
        std::swap(first, second);
    QList<int> distances(first.length() + 1);
    std::iota(distances.begin(), distances.end(), 0);

    for (int index = 0; index < second.length(); index++)
    {
        QList<int> dist = {index + 1};

        for (int idx = 0; idx < first.length(); idx++)
        {
            if (first.at(idx) == second.at(index))
                dist.append(distances.at(idx));
            else
                dist.append(1 + qMin(distances.at(idx), qMin(distances.at(idx + 1), dist.last())));
        }
