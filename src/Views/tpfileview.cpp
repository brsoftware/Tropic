    ...
    m_model(nullptr)
{
    ...
    if (!GET_SETTINGS("App/BrowserDirectory", QString).isEmpty())
    {
        if (QFile(GET_SETTINGS("App/BrowserDirectory", QString)).exists())
        {
            m_model = new TpFileModel(this);
            setModel(m_model);
            m_model->setRootPath(QDir::rootPath());
            setRootIndex(m_model->index(GET_SETTINGS("App/BrowserDirectory", QString)));
        }
    }
...
TpFileModel *TpFileView::fileModel()
{
    if (!m_model)
    {
        m_model = new TpFileModel(this);
...
void TpFileView::onCustomContextMenuRequested(const QPoint &point)
{
    ...
    TpGeneralAction *clearDir =
        new TpGeneralAction(menu, tr("Stop Di&splaying Current Directory"), QKeySequence(), true);
    ...
    [=, this](bool){
        ...
        setModel(m_model);
        DEF_SETTINGS;
        SET_SETTINGS("App/BrowserDirectory", "");
        update();
    });
